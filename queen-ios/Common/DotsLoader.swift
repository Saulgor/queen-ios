...
class DotsLoader : UIView {
    ...
    @IBInspectable var dotsRadius :CGFloat = 4 {
        didSet {
    ...
    func startAnimating() {
        ...
        dotLayers.forEach {
            ...
            $0.add(scaleAnimation(after: offset), forKey: "scaleAnim")
            offset = offset+0.25
        }
    }
    ...
    private func scaleAnimation(after: TimeInterval = 0) -> CAAnimationGroup {
        let scaleUp = CABasicAnimation(keyPath: "fillColor")
        scaleUp.beginTime = after
        scaleUp.fromValue = UIColor.gray.cgColor
        scaleUp.toValue = UIColor.lightGray.cgColor
        scaleUp.duration = 0.3
        scaleUp.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
        
        let scaleDown = CABasicAnimation(keyPath: "fillColor")
        scaleDown.beginTime = after+scaleUp.duration
        scaleDown.fromValue = UIColor.lightGray.cgColor
        scaleDown.toValue = UIColor.gray.cgColor
